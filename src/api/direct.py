...
async def analyze_audio_direct_fast(
    file: UploadFile = File(...),
    speech_context: SpeechContextEnum = Form(default=SpeechContextEnum.CONVERSATIONAL),
    device: str = Form(default="cpu"),
    background_tasks: BackgroundTasks = None,
    ...
    try:
        logger.info(f"[Direct-Fast] Received fast analysis request: {job_id} for {file.filename}")
    except Exception as e:
        logger.error(f"Failed to log request: {str(e)}")
        raise HTTPException(status_code=500, detail="Internal error")
    ...
    return {
        ...
        "status": "queued",
        "mode": "fast",
        "message": f"Fast analysis started. Poll /api/direct/v1/result/{job_id} for results"
    }
...
# ============================================================================
# Background processing function for FAST analysis
# ============================================================================

async def _process_analysis_direct_fast(job_id: str, tmp_path: str, speech_context: str, device: str):
    """
